...
def main():
    ...
    def parse_ai_analysis_output(text: str) -> dict:
        """Parses AI analysis text to extract structured incident report parameters."""
        parsed = {
            ...
            "poi_detected": "Not specified",
            "instruments_detected": "None",
            "summary": text or ""
        }
        if not text:
            return parsed
        ...
        if not json_match:
            json_match = re.search(r"(\{[\s\S]*\"incident_type\"[\s\S]*\})", text)
    
        if json_match:
            try:
                data = json.loads(json_match.group(1))
                valid_types = ["Robbery", "Fighting", "Shooting", "Stealing", "Abuse", "Vandalism", "Suspicious Behavior", "Normal Video"]
                if data.get("incident_type") in valid_types:
                    parsed["incident_type"] = data["incident_type"]
                if data.get("location"):
                    parsed["location"] = str(data["location"])
                if data.get("start_timestamp"):
                    parsed["start_timestamp"] = str(data["start_timestamp"])
                if data.get("end_timestamp"):
                    parsed["end_timestamp"] = str(data["end_timestamp"])
                if data.get("severity") and isinstance(data["severity"], (int, float)):
                    parsed["severity"] = max(1, min(5, int(data["severity"])))
                if data.get("confidence_score") and isinstance(data["confidence_score"], (int, float)):
                    parsed["confidence_score"] = max(0.0, min(100.0, float(data["confidence_score"])))
                if data.get("poi_detected"):
                    parsed["poi_detected"] = str(data["poi_detected"])
                if data.get("instruments_detected"):
                    parsed["instruments_detected"] = str(data["instruments_detected"])
                return parsed
            except Exception:
                pass
    
        # 2. Fallback to Regex & Keyword Heuristics
        lower_text = text.lower()
        if "shooting" in lower_text or "gun" in lower_text or "firearm" in lower_text:
            parsed["incident_type"] = "Shooting"
            parsed["severity"] = 5
            parsed["instruments_detected"] = "Handgun"
        elif "robbery" in lower_text or "armed" in lower_text:
            parsed["incident_type"] = "Robbery"
            parsed["severity"] = 5
        elif "fight" in lower_text or "assault" in lower_text or "punch" in lower_text or "brawl" in lower_text:
            parsed["incident_type"] = "Fighting"
            parsed["severity"] = 4
        elif "steal" in lower_text or "theft" in lower_text or "shoplift" in lower_text:
            parsed["incident_type"] = "Stealing"
            parsed["severity"] = 2
        elif "abuse" in lower_text or "push" in lower_text:
            parsed["incident_type"] = "Abuse"
            parsed["severity"] = 3
        elif "vandal" in lower_text or "graffiti" in lower_text or "smash" in lower_text:
            parsed["incident_type"] = "Vandalism"
            parsed["severity"] = 3
    
        # Extract timestamps: look for pairs of MM:SS
        ts_matches = re.findall(r"\b(\d{2}:\d{2})\b", text)
        if len(ts_matches) >= 2:
            parsed["start_timestamp"] = ts_matches[0]
            parsed["end_timestamp"] = ts_matches[1]
        elif len(ts_matches) == 1:
            parsed["start_timestamp"] = ts_matches[0]
            parsed["end_timestamp"] = ts_matches[0]
    
        # Extract POI hints
        poi_match = re.search(r"(?:poi|person of interest|suspect|individual):\s*([^\.\n]+)", text, re.IGNORECASE)
        if poi_match:
            parsed["poi_detected"] = poi_match.group(1).strip()
    
        # Extract Weapon / Instrument hints
        inst_match = re.search(r"(?:weapon|instrument|object):\s*([^\.\n]+)", text, re.IGNORECASE)
        if inst_match:
            parsed["instruments_detected"] = inst_match.group(1).strip()
    ...
    with header_col1:
        ...
        """, unsafe_allow_html=True)
    with header_col2:
        st.image("src/frontend/assets/nvidia_logo_horizontal.png", width="stretch")
    ...
    if st.session_state.current_page == "reports":
        ...
        if st.session_state.reports_subview == "analyze":
            ...
            with col_setup:
                ...
                struct_suffix = "\n\nAt the end of your response, provide a JSON summary block with keys: incident_type, location, start_timestamp, end_timestamp, severity (1-5), confidence_score (0-100), poi_detected, instruments_detected."
                chip_col1, chip_col2 = st.columns(2)
                with chip_col1:
                    if st.button("robbery / violence", width="stretch"):
                        st.session_state.search_prompt = "Is there any robbery, fighting, physical violence, or safety concerns in this video? Focus on when it happens and who is involved." + struct_suffix
                    if st.button("stealing / theft", width="stretch"):
                        st.session_state.search_prompt = "Look for any suspicious shoplifting, stealing, or concealment of items in this footage. Detail the actions." + struct_suffix
                with chip_col2:
                    if st.button("suspicious activity", width="stretch"):
                        st.session_state.search_prompt = "Analyze the video for general anomalies, suspicious behaviors, or trespassing. Provide timestamps." + struct_suffix
                    if st.button("timeline overview", width="stretch"):
                        st.session_state.search_prompt = "Write a concise timeline of events in this clip. What actions are happening?" + struct_suffix
                ...
                analyze_button = st.button("Run AI Video Analysis", width="stretch")
            ...
            if st.session_state.last_analysis_result:
                ...
                st.write("Review the details extracted from AI model analysis and save to the database.")
    
                parsed_info = parse_ai_analysis_output(st.session_state.last_analysis_result)
                ...
                with st.form("save_report_form"):
                    ...
                    type_index = incident_types.index(parsed_info["incident_type"]) if parsed_info["incident_type"] in incident_types else 6
    
                    save_type = st.selectbox("Detected Incident Type", incident_types, index=type_index)
                    save_loc = st.text_input("Incident Location / Camera ID", value=parsed_info["location"])
    
                    c_ts1, c_ts2 = st.columns(2)
                    with c_ts1:
                        save_start = st.text_input("Start Timestamp (e.g. 00:10)", value=parsed_info["start_timestamp"])
                    with c_ts2:
                        save_end = st.text_input("End Timestamp (e.g. 00:40)", value=parsed_info["end_timestamp"])
    
                    save_sev = st.slider("Severity Level", 1, 5, value=parsed_info["severity"])
                    save_conf = st.slider("Confidence score (%)", 0.0, 100.0, value=parsed_info["confidence_score"])
                    save_poi = st.text_input("Person of Interest (POI) description", value=parsed_info["poi_detected"])
                    save_inst = st.text_input("Instrument / Weapon Used", value=parsed_info["instruments_detected"])
                    save_summary = st.text_area("Incident summary text", value=parsed_info["summary"], height=120)
        ...
        # ------------------------------------------------------------------
            ...
            with col_video:
                ...
                try:
                    ...
                    with ctrl1:
                        if st.button("Play from Incident Start", width="stretch"):
                            st.session_state.video_playback_start = start_seconds
                            st.rerun()
                    with ctrl2:
                    ...
                    start_seconds = timestamp_to_seconds(report['start_timestamp'])
                    end_seconds = timestamp_to_seconds(report['end_timestamp'])
                    max_clip_seconds = max(120, end_seconds + 15) if end_seconds > 0 else 120
    
                    start_pct = min(95.0, max(0.0, (start_seconds / max_clip_seconds) * 100.0))
                    duration_pct = min(100.0 - start_pct, max(3.0, ((end_seconds - start_seconds) / max_clip_seconds) * 100.0))
                    end_pct = min(100.0, start_pct + duration_pct)
                    ...
                    <div style="background-color: #FFFFFF; border: 1px solid #E4E7EC; border-radius: 8px; padding: 12px; margin-top: 16px;">
                        ...
                        <div style="height: 12px; background-color: #E4E7EC; border-radius: 6px; position: relative; width: 100%;">
                            <div style="position: absolute; left: {start_pct:.1f}%; width: {duration_pct:.1f}%; height: 100%; background-color: #76B900; border-radius: 6px;"></div>
                            <div style="position: absolute; left: {start_pct:.1f}%; height: 16px; width: 2px; background-color: #DC2626; top: -2px;"></div>
                            <div style="position: absolute; left: {end_pct:.1f}%; height: 16px; width: 2px; background-color: #DC2626; top: -2px;"></div>
                        </div>
                        <div style="display: flex; justify-content: space-between; font-size: 11px; color: #98A2B3; margin-top: 6px;">
                            <span>00:00 (Start)</span>
                            <span>Incident Range: {report['start_timestamp']} - {report['end_timestamp']}</span>
                            <span>Clip End (~{max_clip_seconds // 60:02d}:{max_clip_seconds % 60:02d})</span>
                        </div>
                    </div>
                    """, unsafe_allow_html=True)
                    ...
                    st.markdown("<div style='margin-top: 16px;'></div>", unsafe_allow_html=True)
                    if st.button("Run AI Analysis on This Video", width="stretch"):
            ...
            with col_review:
                ...
                with st.form("edit_report_form"):
                    ...
                    with btn_col2:
                        confirm_delete = st.form_submit_button("Delete Report", width="stretch")
        ...
        # ------------------------------------------------------------------
            ...
            with upload_col:
                ...
                show_upload = st.button("Upload New Video", width="stretch")
            ...
            if st.session_state.get("show_upload_panel", False):
                with st.container(border=True):
                    ...
                    if uploaded_file is not None:
                        ...
                        with up_col2:
                            if st.button("Confirm Upload", width="stretch"):
                        ...
                        if idx + j < len(filtered_reports):
                            ...
                            with row_cols[j]:
                                with st.container(border=True):
                                    ...
                                    if st.button("View and Verify Details", key=f"select_rep_{r['id']}", width="stretch"):
